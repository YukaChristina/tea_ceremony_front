...
### 1. `user_id = 1` のハードコード（優先度：高）
バックエンドの全SQLクエリで `user_id = 1` が固定されています。
複数人対応の際は、全エンドポイントの修正＋既存データの移行が必要です。
データが積み重なるほど移行コストが上がるため、認証導入と同時に早めに対応することを推奨します。
...
### 3. 認証ミドルウェアの不在（優先度：高・複数人対応時）
現在は認証なしでAPIにアクセスできます。
後から追加すると全エンドポイントへの認証チェック差し込みが必要になります。
**Supabase Authを使えば最小限の変更で導入できるため、複数人対応と同時に実装することを推奨します。**
